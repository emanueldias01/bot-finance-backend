...
async def create(request: OpenFinanceItemRequest, db: Annotated[AsyncSession, Depends(get_session)]):
    try:
...
async def list(db: Annotated[AsyncSession, Depends(get_session)]):
    try:
...
async def get(id: str, db: Annotated[AsyncSession, Depends(get_session)]):
    try:
