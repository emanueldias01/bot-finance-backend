...
async def register_user(data: UserRequest, db: Annotated[AsyncSession, Depends(get_session)]) -> UserResponse:
    return await register(data=data,db=db)

@router.post("/login", response_model=TokenResponse, status_code=200)
async def login_user(data: UserRequest, db: Annotated[AsyncSession, Depends(get_session)]) -> TokenResponse:
